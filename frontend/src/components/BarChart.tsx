...
// Eixo compacto: só usa "k" (milhares) quando o valor realmente chega lá — evita
// mostrar "0k" pra valores pequenos, comuns numa igreja pequena.
...
export function BarChart({ porMes }: { porMes: Record<string, { ent: number; sai: number }> }) {
  ...
  const W = 640, H = 220, padL = 40, padB = 26, padT = 22;
...
export function TrendSparkline({ porMes }: { porMes: Record<string, { ent: number; sai: number }> }) {
  ...
  const W = 560, H = 100, padL = 14, padR = 14, padTop = 24, padBottom = 16;
  ...
  const yFor = (v: number) => padTop + (H - padTop - padBottom) * (1 - (v - min) / (max - min || 1));
